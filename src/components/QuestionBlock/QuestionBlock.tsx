...
const QuestionBlock = ({
  ...
  const isLast = index == 2 ? "rounded-b-2xl " : "";
    ...
    return (
    <div className={`flex flex-col justify-center gap-4 border border-x-0 border-y border-slate-700 ${isFirst} ${isLast}`}>
        ...
        <div className="px-12 py-4">
            {questions.map((item) => {
                ...
                <ItemAccordion 
                    key={item.questionID}
                    questionID={item.questionID} questionTitle={item.questionTitle} questionBody={item.questionBody} 
                />
                )
            })}
        </div>
