...
const About = () => {
  ...
  return (
    <Section className={`z-10 ${defaultSelection} pb-20`} id="sobre" height={"auto"}>
      <motion.div 
      initial={{opacity: 0}}
          ...
          className={`mb-4 flex flex-col gap-2 border-2 border-white ${ubuntu.className}`}>
            ...
            >
              <span>
                ...
                melhores práticas de desenvolvimento, garantindo um resultado que
                une performance e estética. Mais do que apenas entregar linhas de
                código, busco construir parcerias de confiança. Comigo, você terá
                um profissional atento aos mínimos detalhes e focado em resolver
      ...
      <div className={`my-8 max-w-3xl justify-self-center text-center text-lg font-semibold text-zinc-400 `}>
        <motion.h1 
          initial={{opacity: 0}}
          whileInView={{opacity: 1}}
          viewport={{once: true}}
      ...
      <motion.div 
        ...
        {characteristics.map((item, index) => {
          return (
            <CharCard key={index} title={item.title} icon={item.icon} description={item.description} />
          )
        })}        
      </motion.div>
      <GradualBlur
